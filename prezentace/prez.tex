...
\addtobeamertemplate{navigation symbols}{}{%
    ...
    \hspace{1em}%
    \insertframenumber/nevim%\inserttotalframenumber
}
\begin{document}
...
\begin{block}{Fidelita}
  \begin{itemize}
    ...
    \item pravděpodobnost přechodu do nového stavu
...
\frame{
  ...
  \framesubtitle{Jaké jevy lze při řízení parametrických Hamiltonianů pozorovat?}
  \begin{equation}
    \HH(t)=\begin{pmatrix}
        \Omega(t)&\Delta(t)\\
        \Delta(t)&-\Omega(t)
...
\frame{
  \frametitle{Dvouhladinový systém (toy model)}
  \framesubtitle{Geodetický driving}
...
\frame{
  ...
  \begin{block}{Nové odpovědi}
    
    \begin{itemize}
      \item přiblížení energetických hladin vede k oscilacím
      \item význačné finální časy s velkou fidelitou
      \item známe strukturu ground state manifoldu pro Lipkin-Meshkov-Glick model
      \item modelování pomocí Landau-Zenerovy formule a adiabatické perturbační teorie
    \end{itemize}
  \end{block}

\begin{block}{Nové otázky}
  \begin{itemize}
    \item Nalezení trajektorie s maximální fidelitou
    \item korespondence fidelity s energetickou variancí
    \item dokázání diabolických bodů v Lipkin-Meshkov-Glick modelu
  \end{itemize}
\end{block}
}
