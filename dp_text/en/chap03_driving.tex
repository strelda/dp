...
\begin{proof}[\textbf{Proof of the  metric tensor definitions correspondence}]\
...
\subsection{APT}
Following the article from \cite{Rigolin2008}, we approximate the wave-function by series. Because every element will then be decomposed into another series, we will bear in mind is the \emph{locality of variables}. Let's call variable $V(s)$ \emph{\bluee{local}} if it depends only on infinitesimal surrounding of $s$. These variables will be in shades of \bluee{blue} and \redd{non-local} variables in shades of \redd{red}.
...
The problem now lies in determining $\redd{b_n^{(p)}(s)}$, which is also \red{non-local}. Because it depends on its relative \reddd{geometric} and \reddd{dynamical phase} to other \bluee{energy levels}, let's write it as a series
\begin{equation}
...
\subsection{\red{Adiabatic driving}}
Adiabatic transformation is such transformation from $\P\M$ to $\P\M$, which does not excite the system, meaning the fidelity $F=1$. Generally it can be achieved by two methods -- infinitely slow transformation of states, or adding some \emph{counter-diabatic element} to the Hamiltonian.
...
\begin{definition}[Adibaticity]
    Slow change of the driving parameters of the Hamiltonian $H(\llambda)$ in a sense, that it does not excite the system and allows the system to return to the same energetic state after circulation around any \emph{closed path} on the manifold with fidelity $F=1$. 
\end{definition}
...
\subsection{\red{Counterdiabatic driving}}
