...
\begin{equation}
    \mathcal J\coloneqq\{\llambda(t)|t\in[0,T_f],\; \llambda\in \mathcal U\} \subset \R^N,\quad \llambda(0)=\llambda,\; \llambda(T_f)=\tilde\llambda,
...
% This example is taken from \citet{kolodrubez}[page 15--17]. The idea of a counter-diabatic driving is that any excitation of the system can be countered by adding the counter-diabatic potential to the Hamiltonian. Consider again any eigenstate $\ket{\psi(\llambda)}$ of the Hamiltonian $\HH=\HH(\llambda)$ driven along the curve $\mathcal J(\llambda(t))$ on $\P\M_0$ depending on time $t$. Because the system is not measured during the driving, it can't be stated if or if not it was excited, but the main goal here is to achieve fidelity $F=1$, which is iff $\tilde H$ is diagonal along the driving path. For diagonalizable Hamiltonian, there exist a transformation, see Eq. \ref{eq:transformationU}, for which the fidelity will be zero. Such transformation does not have to be unique, but we can choose any one of them. This can be seen more clearly from direct transformation of the Schr\"odinger equation. 
...
Let's have path $\mathcal J$ defined in Eq. \ref{eq:defJ_QSdriving} and a geodesic $\mathcal G(\llambda(t))$ with fixed boundary conditions
 $$\mathcal{G}(0)=\mathcal J(0),\qquad \mathcal{G}(T_f)=\mathcal J(T_f).$$
...
\begin{equation}
    s_{\mathcal J}=\int\limits_{\mathcal J}\d s=\int\limits_{0}^{T_f}\sqrt{g_{jk}\dot\llambda^j\dot\llambda^k}\d t
...
\begin{thm}
    ...
    \begin{equation}
        \int\limits_0^{T_f}\sqrt{\delta E^2(t)}\d t \eqqcolon l_t\geq l_\lambda \coloneqq \int\limits_{\lambda_i}^{\lambda_f} \sqrt{g_{\lambda\lambda}\d \lambda\d \lambda} =\int_0^{T_f} \sqrt{g_{\lambda\lambda}}|\dot \lambda|\d t.
    \end{equation}
...
Following the article from \cite{Rigolin2008}, the wave-function can be approximated by series. Because every element is then decomposed into another series, we bear in mind the \emph{locality of variables}, which clears the reason for performing this procedure. Let's call variable $V(t)$ \emph{\bluee{local}} if it depends only on infinitesimal surrounding of its parameter $t\in\R$. These variables are in shades of \bluee{blue} and \redd{non-local} variables in shades of \redd{red}.

We are interested in the driving along path $\mathcal J$ defined by Eq. \ref{eq:defJ_QSdriving},
where $t$ is time and $T_f$ final time of the driving. Let the initial condition be
\begin{equation}
