...
In this chapter, we deal with the system described by the finite-dimensional Hamiltonian $\HH(\llambda)$ which drives the system according to the Schr\"odinger equation from some initial state $\ket{s(\llambda}$ to $\ket{s(\tilde\llambda)}$ along the path $\gamma(\lambda)$. Before going through the details of adiabatic transformations, let's define its meaning properly.
...
Following the article from \cite{Rigolin2008}, we approximate the wave-function by series. Because every element will then be decomposed into another series, we will bear in mind the \emph{locality of variables}. Let's call variable $V(t)$ \emph{\bluee{local}} if it depends only on infinitesimal surrounding of its parameter $t\in\R$. These variables will be in shades of \bluee{blue} and \redd{non-local} variables in shades of \redd{red}.
