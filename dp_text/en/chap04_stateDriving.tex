...
In this chapter, we deal with the system described by the finite-dimensional Hamiltonian $\HH(\llambda)$ which drives the system according to the Schr\"odinger equation from some initial state $\ket{s(\llambda}$ to $\ket{s(\tilde\llambda)}$ along the path $\mathcal J(\lambda)$. Before going through the details of adiabatic transformations, let's define its meaning properly.
...
% \begin{equation}
%     \ket{\psi(\llambda)} = \sum_{m,n}\psi_n(\llambda) \ket{m(\llambda)}\overbrace{\braket{m(\llambda)|n}}^{U_{ms}(\llambda)} =\sum_m \overbrace{\tilde{\psi}_m(\llambda)}^{\braket{m(\llambda)|\psi_n|n}}\ket{m(\llambda)},
% \end{equation}
...
Following the article from \cite{Rigolin2008}, we approximate the wave-function by series. Because every element is then decomposed into another series, we bear in mind the \emph{locality of variables}. Let's call variable $V(t)$ \emph{\bluee{local}} if it depends only on infinitesimal surrounding of its parameter $t\in\R$. These variables are in shades of \bluee{blue} and \redd{non-local} variables in shades of \redd{red}.
