...
Fidelity during the transport is then\footnote{If we would calculate the fidelity in the \bluee{comoving frame}, we would get exactly one. This realization leads to the counterdiabatic driving.}
\begin{equation}
...
\subsection{\red{Energy variance}}
Another interesting quantity is the energy variance. It has meaning in the \red{Polkovnikov theorem. I might test it here.}
...
Using this we can describe the driving using three regimes\footnote{The coefficient $p$ is assumed to be small enough not to cover the biggest oscillations after $t=T_f/2$ and big enough to average over suffitient number of oscillations. Approximately $p\in[0.6T_f,0.999T_f]$}:
\begin{itemize}
...
\end{figure}
\subsubsection{\red{Oscillatory part from APT}}

\subsubsection{Exponential part from Landau-Zener}
Landau-Zener-Stueckelberg theory provides the WKB approximation formula for the fidelity during some transport. Full form can be seen in \cite{nonadiabaticTransition}. In our case, only simplified theorem is needed. 
\begin{thm}[Landau-Zener-Stueckelberg for linear driving]
    ...
    \end{itemize}
    the fidelity can be described for times $(0,T_c)$ as
    \begin{equation}
        F = \exp\left(-\frac{2 \pi A^2}{\hbar v |\Delta F}\right),
    \end{equation}
    for a \emph{diabatic coupling} $A$ and difference between \emph{diabatic potentials} $\Delta F$. 
\end{thm}

In our case, $v=1/T_f$ and $2 A^2/|\Delta F|=\Omega_i^2/\Delta_{sc}$.
...
\newpage
\subsection{\red{Energy variance}}
Energy variance resembles structure similar to fidelity. Compare Fig. \ref{fig:densVariance} with \ref{fig:dens3}. \red{why is it almost the same as fidelity of geodesical driving???}
\begin{figure}[H]
    \centering
