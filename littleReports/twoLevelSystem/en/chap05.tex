...
\begin{align}
    ...
    \label{eq:frequency}
\end{align}
Along with normalization condition \ref{eq:normalizationCondition}, this solves the Schr\"odinger equation. Note that in this form $\Delta\neq 0$. This is not problem due to system symmetry $\Delta\leftrightarrow \Omega$, which means we can change the driving by interchanging $\Delta$ and $\Omega$.
...
From linear driving definition and Eq. \ref{eq:energy} we have
\begin{equation}
...
\begin{align}
    \gamma(t) &= 0\\
    \omega^2(t)&=i\frac{\Omega(t)}{t}+\frac{E^2_{dif}}{4}\label{eq:oscillationsLinear}
\end{align}


\subsection{Dependence on time}
The fidelity in time can be seen on Fig. \ref{fig:infidelityTimePlotLin}. We can see that for $t\approx T_f/2$ Hamiltonian parameters change quickly which leads to the state excitation. Then the Harmonic oscillator damping of Schr\"odinger equation gets involved, and oscillations are quickly going to zero, never disappearing entirely.

We can see that the final fidelity decreases with longer final time, which correctly leads to adiabatic driving $\lim_{T_f\rightarrow \infty} F=0$. For short final times we can observe so called quench, $\lim_{T_f\rightarrow 0} F=1$. The important phenomenon on this image are the oscilations around $t=T_f/2$, which are more intense with longer final time.
\begin{figure}[H]
    \centering
    \includegraphics[scale=1.185]{../img/infidelityInTimePlot.pdf}
    \caption{Infidelity in time for three final times $T_f\in\{1,10,100\}$ for the linear driving.}
  \label{fig:infidelityTimePlotLin}
\end{figure}

If the driving is slow enough (meaning of this will be clear later on), the final fidelity will be oscilating between $0$ and some small value, see Fig. \ref{fig:slowinfidCombined}.
\begin{figure}[H]
    \centering
...
One would like to analyze these oscillations from $\omega^2(t)$ described by Eq. \ref{eq:oscillationsLinear}. The problem is that we are observing the fidelity$=\braket{0(t)|\psi(t)}$ where $\bra{0(t)}\neq const.$ and might be oscillating as well.
\begin{equation}
    \textcolor{red}{?calculation\; of\; \ket{0(t)}\; oscillation?}
\end{equation}


\subsection{Dependence on final time}
Because the oscillations after fast parameter change in the Hamiltonian never disappear entirely, we must observe those oscillations even at the final time.\emph{Final fidelity} (meaning the fidelity at final time $F(T_f)$) has dependence on $T_f$ as can be seen in Fig. \ref{fig:infidelityTfPlotLogLin}. Because after the final time $T_f\approx 2000$ the values are so small, we can observe some fine structure of the fidelity, along with numerical error artifacts.
\begin{figure}[H]
    \centering
    \includegraphics[scale=1.2]{../img/infidelityTfPlotLogLin.pdf}
    \caption{Final infidelity as a function of $T_f$.}
    \label{fig:infidelityTfPlotLogLin}
\end{figure}

First of all, the numerical precision of this calculation was found to be around $10^{-15}$. This means that the oscillations we see on Fig. \ref{fig:infidelityTfPlotLogLin} are of physical origin with some additional numerical error. In fact these small oscillations are the remnants of the fast excitation during close approach of energy levels, see Fig. \ref{fig:infidelityTimePlotLin}. We can eliminate the effect of these oscillations by averaging over some time. For that we define the \emph{average final infidelity}
\begin{equation}
...
\begin{itemize}
    ...
    \item \emph{transitional regime} -- \textcolor{red}{smooth? or sharp?}
    \item \emph{polynomial/adiabatic regime} -- $\langle 1-F\rangle_p\propto T_f^{-\kappa}$ for $\kappa\in \R^+$.
\end{itemize}

Two questions now arise. What are the coefficients $\xi$ and $\kappa$ and on which intervals in $T_f$ these transitions hold? Let's answer them one by one.


\begin{figure}[H]
    \centering
    \includegraphics[scale=1.2]{../img/infidCombined.pdf}
