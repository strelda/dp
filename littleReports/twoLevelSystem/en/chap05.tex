...
\end{equation}
with time varying Hamiltonian. For 2-dimentional system with $\ket{\psi(t)}\eqqcolon (a(t),b(t))$ we get system of \emph{two coupled differentials equations of the 1. order with non-constant coefficients}
\begin{align}
...
The Equations \ref{eq:a1}, \ref{eq:a2} have no general analytical solution, with the exceptions of a few easy protocols $\HH(t)$. Those equations can be rewritten to \emph{one differential equation of 2. order with non-constant coefficients}, which can be rewritten in a form of  a \emph{damped harmonic oscillator without external force}
\begin{align}
    0&= \ddot a(t)+ \gamma(t) \dot a(t)+\omega^2(t)a(t) \label{eq:harmonicOscillator}\\
    \gamma(t)&\coloneqq i\Omega(t)(1-\Delta(t))-\frac{\dot \Delta(t)}{\Delta(t)}\\
    \omega^2(t) &\coloneqq i\dot\Omega(t)-i\Omega(t)\frac{\dot\Delta(t)}{\Delta(t)}-\Delta(t)\Omega^2(t).
    \label{eq:frequency}
\end{align}
Along with the normalization condition \ref{eq:normalizationCondition}, this solves the Schr\"odinger equation. Note that in this form $\Delta\neq 0$, but we can switch role of $\Delta$ and $\Omega$ driving due to system symmetry and the fidelity will be unchanged. 

The solution of Eq. \ref{eq:harmonicOscillator} leads to damped oscillations. This characteristic will propagate to the solution for fidelity.




\subsubsection{Classical mechanics correspondence}
From the perspective of classical mechanics, meaning $x(t)oloneqq a(t)$ is a position in a phase space $(\bm x,\bm p)$, we can write classical Lagrangian from Eq. \ref{eq:harmonicOscillator} as
\begin{equation}
...
\begin{proof}
    ...
    \begin{equation}
        ...
        \end{split}
    \end{equation}
    where derivation along upper bound $F(x)\coloneqq\int_0^{g(x)}f(t)\d t \Rightarrow F'(x)=f(g(x))g'(x)$ for $f(x)\in L^1(0,g(x))$ and differentiable function $g$, was used.
\end{proof}
...
In this case we define the driving as
\begin{equation}
    \Omega(t)=\Omega_{sc}\left(\frac{2t}{T_f}-1\right),\quad \Delta(t)=\Delta_{sc}, \;\;\text{ for } \Omega_{sc}=10, \Delta_{sc}=0.3
\end{equation}
\begin{figure}[H]
    \centering
    \includegraphics[scale=1.2]{../img/drivingLin.pdf}
    \caption{Driving along the linear path. $\lambda_i=(-10;0.3)$ and $\lambda_f=(10;0.3)$ are initial resp. final parameters. DensityPlot shows the difference between Hamiltonian eigenvalues.}
    \label{fig:driving}
\end{figure}
...
The fidelity in time can be seen on Fig. \ref{fig:infidelityTimePlotLin}. We can see that for $t\approx T_f/2$ Hamiltonian parameters change quickly leading to the fast state excitation. Then the Harmonic oscillator damping of Schr
\"odinger equation gets involved, and oscillations are quickly going to zero, never disappearing entirely.

We can see that the final fidelity decreases with longer final time, which correctly leads to adiabatic driving $\lim_{T_f\rightarrow \infty} F=0$. For short final times we can observe so called quench, $\lim_{T_f\rightarrow 0} F=1$. The important phenomenon on this image are the oscilations around $t=T_f/2$, which are increasing with $T_f$.
\begin{figure}[H]
    \centering
    \includegraphics[scale=1.185]{../img/infidelityInTimePlot.pdf}
    \caption{Infidelity in time for three final times $T_f\in\{1,10,100\}$ for linear driving.}
  \label{fig:infidelityTimePlotLin}
\end{figure}

If the driving is slow enough (meaning of this will be clear later on), the final fidelity will oscilating between $0$ and some small value, see Fig. \ref{fig:slowinfidCombined}. Because the eigenstate $\ket{0(t)}=\begin{pmatrix}
    1&0
\end{pmatrix}$, the frequency of oscillations in fidelity is the same as oscillations in $a(t)$ in Eq. \ref{eq:frequency}. For the linear driving this is
\begin{equation}
    \omega^2(t) = \frac{2i}{T_f}\Omega_{sc}-\Delta_{sc}\Omega_{sc}^2\left(\frac{2t}{T_f}-1\right)^2.
\end{equation}
This means the frequency is linear in time and has a square root dependence on $\Delta$.



\begin{figure}[H]
    \centering
    \hspace{40pt}\includegraphics[scale=1.185]{../img/slowinfidCombined.pdf}
    \vspace{-40pt}
    \caption{Infidelity in time for close adiabatic driving.}
  \label{fig:slowinfidCombined}
\end{figure}


\subsection{Dependence on final time}
Because the oscillations after fast parameter change in the Hamiltonian never disappear entirely, we must observe those oscillations even at the final time. Final fidelity (meaning the fidelity at final time $F(T_f)$) has dependence on $T_f$ as can be seen in Fig. \ref{fig:linear_finalInfidelity_log}. Because after the final time $T_f\approx 2000$ the values are so small, we can observe some fine structure of the fidelity, along with numerical error artifacts.
\begin{figure}[H]
    \centering
    \hspace{40pt}\includegraphics[scale=1.2]{../img/infidelityTfPlotLogLin.pdf}
    \vspace{-40pt}
    \caption{Final infidelity as a function of $T_f$.}
    \label{fig:linear_finalInfidelity_log}
\end{figure}

The important question is now: "what is numerical error and what is remnant of oscillations?" The answer is not so simple, but we can make few statements. First of all, the numerical precision of this calculation was foud to be around $10^{-14}$. This can be seen when we zoom at the edge of \emph{smooth} and \emph{chaotic} part of the plot, see Fig \ref{fig:infidelityZoom} \footnote{First zero probably appears at 1889, which is the same year when the Eiffel tower was oficially opened, or the year when the first Juke box goes into operation in San Francisco.}. This means that there are oscillations are of physical origin. Those small oscillations are the remnants of the big oscillations after fast fidelity change, see Fig. \ref{fig:infidelityTimePlotLin}. We can eliminate the effect of these oscillations and see the \emph{average final infidelity}, defined as
\begin{equation}
    \langle 1-F\rangle_a(T_f) \coloneqq \frac{1}{(1-a) T_f}\int_{a T_f}^{T_f} F(t)\d t.
\end{equation}
If we average the fidelity on interval $(0.9 T_f,T_f)$ (getting $\langle 1-F\rangle_{0.9}$), we get the same result as for $\langle 1-F\rangle_{0.99}$. See Fig. \ref{fig:infidAverageCombined}.


Then we can split the driving to three regimes:
\begin{itemize}
    \item \emph{exponential/fast-driving regime} -- $(1-F)=\langle 1-F\rangle_a= \exp(-\xi T_f)$, $\xi\in\R^+$
    \item \emph{transitional regime} -- the infidelity dependence is oscilatory and has no simple prescription
    \item \emph{polynomial/adiabatic regime} -- $\langle 1-F\rangle_a\propto T_f^{-\kappa}$ for $\kappa\in \R^+$, $a\tilde\in[0.6T_f,0.999T_f]$.
\end{itemize}

Two question now arise. What are the coefficients $\xi$ and $\kappa$ and on which intervals in $T_f$ they hold? Let's answer them one by one.


\begin{figure}[H]
    \centering
    \includegraphics[scale=1.2]{../img/infidAverageCombined.pdf}
    \caption{Final infidelity as a function of $T_f$. Detail on the edge between \emph{smooth} and \emph{chaotic} regimes.}
    \label{fig:infidAverageCombined}
\end{figure}




\subsection{Transition between the fast-driving and adiabatic regime}
...
\begin{figure}[H]
    ...
    \caption{Energy variance for $\Omega=0.2$ for linear driving. Note that only $t<T_f$ has physical meaning.}
